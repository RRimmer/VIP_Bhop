...
Плагин написан под SourceMod и может работать в других играх на движке Source, если доступны SourceMod и VIP-Core. 
Возможные кандидаты: CS:S, TF2, DoD:S, HL2DM и другие моды Source.  
В CSS плагин работает как обычный бхоп и зависит от ping (анти-стик не активен).
...
- Персональная перезарядка для групп через значение `bhop` (берется целая часть)
- Учет `mp_freezetime` при старте раунда
- Alert/чат уведомления с переводами
- Настройка адресатов уведомлений (все или только VIP)
- Отключение бхопа после урона от противника: `bhop_restrict_type`, `bhop_restrict_time`
- Отключение в первом раунде (с поддержкой `mp_halftime`): `bhop_first_round_off`
...
Можно задать персональную перезарядку для группы через значение фичи `bhop`.
Целая часть числа используется как секунды перезарядки (например, `5.0` = 5 сек).
Значение `1` использует глобальный `bhop_locktime`.
...
| `bhop_restrict_type` | 0 | Отключать бхоп после урона от противника (0/1) |
| `bhop_restrict_time` | 2 | На сколько секунд отключать бхоп после урона |
| `bhop_first_round_off` | 0 | Отключать бхоп в первом раунде (0/1) |
